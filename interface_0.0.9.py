import moneywagon
from states import GetPrice, GetEmail
...
@dp.message_handler(commands=['account', 'инфо'])
async def process_account_command(message):
    db_sess = db_session.create_session()
    is_user_in_db = [user for user in db_sess.query(User).filter(User.id == message.from_user.id)]
    if is_user_in_db:
        btc_wallet = [user.bitcoin_wallet for user in
                      db_sess.query(User).filter(User.id == message.from_user.id)]
...
def register_mail_price(dispatcher):
